...
export default function Achievements() {
    ...
    const loadProjects = async () => {
        try {
            ...
            console.error("Error from Projects.tsx: ", err);
        }
    }
    ...
    if (!projects) return <div className="min-h-screen flex flex-col items-center p-4 gap-y-45" id="projects">
        <h2 className="border-b p-3 text-3xl font-semibold text-center">PROJECTS</h2>
            <Card className="max-w-md w-full shadow-lg bg-black border-0">
                <CardHeader className="flex flex-col items-center text-center space-y-2">
                    ...
                    <CardTitle className="text-3xl text-violet-700 animate-pulse">Loading Projects...</CardTitle>
                </CardHeader>
    ...
    return <div id="projects" className="min-h-screen flex flex-col items-center p-4 gap-y-45">
        <h2 className="border-b p-3 text-3xl font-semibold text-center">PROJECTS</h2>
            ...
            {projects?.map((project: Project) => {
                return (
                    <Card key={project.id} className="w-80 h-80 bg-black/50 border-0 hover:-translate-y-2 transition ease-in duration-300 text-xl">
                        ...
                        <CardFooter>
                            <Button className="cursor-pointer bg-violet-700 text-black hover:bg-violet-500 transition ease-in duration-300 text-lg p-5" onClick={() => handleSeeProject(project.id)}>Know more</Button>
                        </CardFooter>
                    </Card>
                );
            })}
